...
bool DBSqlite::exist(std::string tableName){
	...
	sqlite3_exec(_pdb, sql.str().c_str(), isExisted, &tableIsExisted, &errMsg);
...
void DBSqlite::upgrade(std::string tableName, std::string tableSql){
	...
	webdevlib::trim(oldSql);
	webdevlib::trim(newSql);
	if (oldSql != newSql){
		sql.clear();
		sql.str("");
...
void DBSqlite::upgrade(std::string tableName, std::string tableSql, std::string dataCSV){
	std::stringstream sql;
	sql << "SELECT TRIM(GROUP_CONCAT(sql, ';')) AS `sql` FROM `sqlite_master` WHERE `tbl_name` = '" << tableName << "' ORDER BY type='index', type='table' LIMIT 1";
	Value one = getOne(sql.str());
	std::string oldSql = one.asString();
	std::string newSql = tableSql;
	csv::Parser file = csv::Parser(dataCSV, csv::DataType::ePURE);
	std::string dataSql = file.printSql(tableName);
	if (oldSql == ""){
		query(tableSql);
		query(dataSql);
		return;
