    ...
    for (SKPaymentTransaction *transaction in transactions)
    {
        ...
        switch (transaction.transactionState)
        {
           ...
           //     NSString* imei = [[UIDevice currentDevice].identifierForVendor UUIDString];
                std::string imei = getDeviceID_IOS();
                _iosiap->jsonimei.push_back(imei);
                NSString* jsonObjectString =[self encode:(uint8_t*)transaction.transactionReceipt.bytes length:transaction.transactionReceipt.length];
                _iosiap->jsonimei.push_back([jsonObjectString UTF8String]);
                break;
            }
...
std::string IOSiAP::require_imei()
{
    //   std::string str = jsonimei.at(0);
//    CCLOG("str = %d", jsonimei.size());
    if (jsonimei.size() != 0)
    {
        return jsonimei.at(jsonimei.size() - 2);;
    }
    else
    {
        return "";
...
std::string IOSiAP::require_receiptdata()
{
    //   std::string str = jsonimei.at(0);
  //  CCLOG("str = %d", jsonimei.size());
    if (jsonimei.size() != 0)
    {
        return jsonimei.at(jsonimei.size() - 1);
    }
    else
    {
        return "";
